...
adHuntingResult = []
potentialAdResult = []
# Mobile resolution  640*360 (Default test model: Galaxy S5)
MOBILE_HEIGHT = 640
MOBILE_WIDTH = 360
MOBILE_LARGE_THRESHOLD = 192
# Default desktop resolution 1366*768
DESKTOP_HEIGHT = 768
DESKTOP_WIDTH = 1366
DESKTOP_LARGE_THRESHOLD = 231
...
def killChromeAndChromedriver_linux(onlyKillChromedriver=False):
    try:
        ...
        if ENABLE_DEBUG:
            print('[G]chrome process(es) killed')
        for line in os.popen("ps ax | grep " + 'chromedriver' + " | grep -v grep"):
            fields = line.split()
            pid = fields[0]
            os.kill(int(pid), signal.SIGKILL)
        if ENABLE_DEBUG:
            print('[G]chromedriver process killed')
    except:
        print('[G]Fail to terminate previous processes, program may become less stable!')
...
def AdHuntingOnce(url):
    ...
    killChromeAndChromedriver_linux()
